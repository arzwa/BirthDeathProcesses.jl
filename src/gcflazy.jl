# Generalized continued fractions using lazy lists
# This is super slow
const TINY = 10^-30
ifzerotiny(x::T) where T = iszero(x) ? T(TINY) : x

"""
    LazyContinuedFraction(b₀, a, b)
...
A generalized contiued fraction represented by Lazy lists.
    b₀ +       a₁
        ...
        b₁ +     a₂
            ...
            b₂ +  a₃/...
"""
struct LazyContinuedFraction{T}
    b₀::T
    a ::List
    b ::List
end

Base.show(io::IO, gcf::LazyContinuedFraction{T}) where T =
    write(io, "LazyGCF{$T}: $(terms_string(gcf, 2))")

function terms_string(gcf::LazyContinuedFraction, n)
    str = "($(gcf.b₀) + "
    for i=1:n
        str *= "$(gcf.a[i]) / ($(gcf.b[i]) + "
    end
...
"""
    lentz(c::LazyContinuedFraction)

Modified Lentz method, as in section 5.2 of 'Numerical recipes in C'.
"""
function lentz(gcf::LazyContinuedFraction{T}, ϵ=10^-7, nmax=1000) where T
...
# FIXME
# function _lentz(gcf::LazyContinuedFraction{T}, ϵ=10^-7, nmax=1000) where T
#     C = f = ifzerotiny(gcf.b₀)
#     Δ = Inf
...
"""
    convergents(gcf::LazyContinuedFraction, k::Int)
...
"""
function convergents(gcf::LazyContinuedFraction{T}, k::Int) where T
...
function _convergents(gcf::LazyContinuedFraction{T}, k::Int) where T
    AB = zeros(Complex, k, 2)
    A = C = ifzerotiny(gcf.b₀)
    B = one(T)
    D = zero(T)
    for j=1:k
        D = one(D)/ifzerotiny(gcfb[j] + gcfa[j]*D)
        C = ifzerotiny(gcfb[j] + gcfa[j]/C)
        AB[j, 1] = A = A*C
        AB[j, 2] = B = B/D
    end
    return AB
